...
std::vector<sycl::device> enumerate_devices(const int n_samples_template,
					    const int n_samples_data,
					    const int n_templates,
    ...
    const size_t total_memory = sizeof(float) *
    ...
    const size_t local_memory = sizeof(float) * network_size * 2
	+ sizeof(int) * network_size
	...
	const auto dev_local_mem = device.get_info<sycl::info::device::local_mem_size>();
...
int matched_filter_sycl(const float* templates, const float* sum_square_templates, const int* moveouts, const float* data,
    ...
    const auto devices = enumerate_devices(n_samples_template, n_samples_data,
					   n_templates, n_stations, n_components, n_corr);
    std::vector<sycl::queue> queues;
    if (devices.size() > 0) {
	// If we have more devices than templates we limit the number of
	// devices to reduce the initialization cost
	for (int i = 0; i < std::min(static_cast<int>(devices.size()), n_templates); i++) {
	...
	device_data.push_back(
		...
		sycl::malloc_device<float>(static_cast<size_t>(n_corr * n_templates), Q),
	    }
	);
	...
	if (queue_mem.templates == nullptr ||
	    queue_mem.sum_sq_templates == nullptr ||
	    queue_mem.data == nullptr ||
	    queue_mem.weights == nullptr ||
	    queue_mem.moveouts == nullptr ||
		...
		std::cerr << "\033[0;31mCRITICAL: " <<
			...
			" with total memory " << device.get_info<sycl::info::device::global_mem_size>() <<
	...
	Q.copy<float>(weights + network_offset, device_mem.weights,
	       network_size);
	Q.copy<int>(moveouts + network_offset, device_mem.moveouts,
	     network_size);
	...
	const size_t local_size = 1024; // Fixed work-group size, ensure it is a multiple of 32
	...
	Q.submit([&](sycl::handler& h) {
	    ...
	    auto local_data = sycl::local_accessor<float, 1>(n_samples_template + local_size, h);

	    h.parallel_for(sycl::nd_range{sycl::range{global_size}, sycl::range{local_size}},
		    [=](auto& idx) {
		...
		for (int station = 0; station < n_stations; station++) {
			...
			const int data_offset = comp_offset * n_samples_data + local_moveouts[comp_offset];

			// We need to synchronize with the previous iteration
			// so that no in-use local memory is overwritten
			idx.barrier(sycl::access::fence_space::local_space);
			for(int i = local_index; i < n_samples_template; i += local_size) {
			    local_templ[i] = device_mem.templates[temp_offset + i];
			}
			for(int i = 0; i + local_index < n_samples_template + local_size; i += local_size) {
			    local_data[local_index + i] = device_mem.data[index + data_offset + i];
			}
			idx.barrier(sycl::access::fence_space::local_space);
			...
			if (normalize) {
				...
				_mean[0] += local_data[local_index + n_samples_template - 1];
			    }
			    const float nstf = static_cast<float>(n_samples_template);
			    mean = (_mean[0] + _mean[1]) / nstf;
			}
			for (int i = 0; (i + 1) < n_samples_template; i+=2) {
			    const auto sample = sycl::float2{local_data[local_index + i] - mean,
							     local_data[local_index + i + 1] - mean};
			    const auto templ = sycl::float2{local_templ[i], local_templ[i + 1]};
			    // Using MAD instead of FMA for speed, exchange if more accuracy is needed
			...
			// that we process the last element separately below
			if (n_samples_template % 2 != 0) {
			    const float sample = local_data[local_index + n_samples_template - 1] - mean;
			...
			float denominator = local_sum_sq[comp_offset] * (sum_square[0] + sum_square[1]);
			if (denominator > STABILITY_THRESHOLD) {
			    denominator = sycl::rsqrt(denominator);
			    // Using MAD instead of FMA for speed, exchange if more accuracy is needed
			    const float numer = numerator[0] + numerator[1];
			    cc_sum = sycl::mad((numer * denominator), local_weights[comp_offset], cc_sum);
			}
		    }
		}
	...
	auto device_mem = device_data[queue_index];
        ...
        const int start_i = static_cast<int>(std::ceil(std::abs(min_moveout)
						       / static_cast<float>(step)) * step);
        const int stop_i = 1 + (n_samples_data - n_samples_template - max_moveout);
	Q.copy<float>(&device_mem.cc_sum[cc_sum_offset + start_i], &cc_sum[cc_sum_offset + start_i], stop_i - start_i);
    }
